...
library(data.table)
f <- fread("bqc19-v4.0-qc4.sexcheck")
png("XChromosomeHomogozyosityEstimate.png", width=600, height = 400)
hist(f$F, breaks=100, xlab="X Chromosome Homogozyosity Estimate", main="All sample")
dev.off()
f1 <- fread("bqc19-v4.0-qc5.sexcheck")
f1 <- f1[,c(1,7)]
f <- f[,c(1:6)]
...
map <- fread("/home/richards/tomoko.nakanishi/09.COVID19/data/05.BQC/01.genotype/v4.0/01.batch/sampleid.studyid.sex.map", header=F)
