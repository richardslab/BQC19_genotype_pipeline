...
s$ADMIXTURE_POP <- as.character(s$ADMIXTURE_POP)
s <- s %>% mutate(final_POP = ifelse(confident == 1, ADMIXTURE_POP, "oth"))
fs <- merge(f, s, by.x=c("V1"), by.y=c("FID"))
png(file="01_misshet.png", width=800, height=600, res=150)
...
mean(fs$V3[fs$final_POP == "eur"])
mean(fs$V3[fs$final_POP == "eur"]) + 5*sd(fs$V3[fs$final_POP == "eur"])
...
png(file="02_misshet_corrected.png", width=800, height=600, res=150)
...
roh <- fread("bqc19-v4.0-qc4.hom.indiv") %>% select(c("FID","KB"))
fs <- fs %>% merge(roh, by.x="V1", by.y="FID")
png(file="03_het_roh.png", width=800, height=600, res=150)
